	...
	return (
		<div className="">
			<div className="w-12 aspect-square rounded-lg relative overflow-hidden bg-white/10">
				...
				<button aria-label="Menu" onClick={() => setShowMenu(prev => !prev)} className="absolute inset-[1px] rounded-lg bg-gray-950"><div className="w-full h-full grid place-content-center rounded-lg">{showMenu ? Close : Hamburger}</div></button>
			</div>
					...
					<div ref={menu} className="p-6 w-full flex flex-col gap-12 bg-gray-950 border-y border-white/10 rounded-b-lg">
						<nav>
							<ul className="flex flex-col gap-1 text-white/60">
								<li>
									<Link onClick={() => setTimeout(() => setShowMenu(false), 100)} href="/en" className={`${pathname === '/en' ? 'text-white bg-white/10' : ''} flex items-center gap-3 w-full px-4 py-2 rounded-lg hover:text-white duration-200`}>
										{Home}
										Home
									</Link>
								</li>
								<li>
									<Link onClick={() => setTimeout(() => setShowMenu(false), 100)} href="/en/projects" className={`${pathname === '/en/projects' ? 'text-white bg-white/10' : ''} flex items-center gap-3 w-full px-4 py-2 rounded-lg hover:text-white duration-200`}>{Briefcase}Projects</Link>
								</li>
								<li>
									<Link onClick={() => setTimeout(() => setShowMenu(false), 100)} href="/en/about" className={`${pathname === '/en/about' ? 'text-white bg-white/10' : ''} flex items-center gap-3 w-full px-4 py-2 rounded-lg hover:text-white duration-200`}>{User}About me</Link>
								</li>
							</ul>
						</nav>

						<ul className="flex flex-col gap-1">
							<h5 className="text-white/60 font-medium text-sm mx-3 my-1 uppercase tracking-wider">Contact</h5>
							<li>
						...
						<div className="mt-auto">
							<h5 className="text-white/60 font-medium text-sm mx-3 my-1 uppercase tracking-wider">Language</h5>
							<LangSelector />
						</div>
					</div>
				</div>
